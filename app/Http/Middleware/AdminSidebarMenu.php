            ...
            if (auth()->user()->can('product.view') || auth()->user()->can('product.create') ||
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        {
                            $sub->url(
                                action([\App\Http\Controllers\BoulevardImportDataController::class, 'index']),
                                __('lang_v1.import_blvd_products'),
                                ['icon' => '', 'active' => request()->segment(1) == 'import-boulevard-products']
                            );
                            $sub->url(
                                action([\App\Http\Controllers\BoulevardImportDataController::class, 'indexServices']),
                                __('lang_v1.import_blvd_services'),
                                ['icon' => '', 'active' => request()->segment(1) == 'import-boulevard-services']
                            );
                        }
                    },
                    ['icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
            ...
            if (in_array('expenses', $enabled_modules) && (auth()->user()->can('all_expense.access') || auth()->user()->can('view_own_expense'))) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('expense.add')) {
                            $sub->url(
                                action([\App\Http\Controllers\ExpenseController::class, 'expenseImport']),
                                __('lang_v1.import_expense'),
                                ['icon' => '', 'active' => request()->segment(1) == 'import-expense' && request()->segment(2) == null]
                            );
                        }
                    },
                    ['icon' => ' <svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        ...
                        if (auth()->user()->can('commission_report.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'getCommissionReport']),
                                __('report.commission'),
                                ['icon' => '', 'active' => request()->segment(2) == 'commission-report']
                            );
                        }
                        if (auth()->user()->can('margin_report.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'getMarginReport']),
                                __('report.margin'),
                                ['icon' => '', 'active' => request()->segment(2) == 'margin-report']
                            );
                        }
                        if (auth()->user()->can('weekly_margin_report.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'getWeeklyMarginReport']),
                                __('report.weekly-margin'),
                                ['icon' => '', 'active' => request()->segment(2) == 'weekly-margin-report']
                            );
                        }
                        if (auth()->user()->can('time_clock_report.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'getTimeClockReport']),
                                __('report.time_clock'),
                                ['icon' => '', 'active' => request()->segment(2) == 'time-clock-report']
                            );
                        }
                        if (auth()->user()->can('services_report.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'getServicesReport']),
                                __('report.services'),
                                ['icon' => '', 'active' => request()->segment(2) == 'services-report']
                            );
                        }
                    },
                    ['icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
